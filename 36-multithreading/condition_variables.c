...
int run(void *arg) {
  ...
  for (;;) {
    ...
    while (value_count < VALUE_COUNT_MAX) {
      ...
      // cond_wait() unlocks specified mutex so other threads can
      // enter the critical section and gets crrnt thread to sleep
      // until cond_signal'd or cond_broadcast'd
      // When signaled/broadcasted, a cond_wait'd thread reacquires lock
