 ...
 * */
/**
 * @brief Clears fields on Account struct
 *
 * @param acc Account to clear
 */
...
static Account *acc_find_by_email(wchar_t *email, Account *acc) {
  ...
  for (int i = 0; i < num_accs; i++) {
    ...
    if (matches) {
      ...
      wcsncpy(acc->password, iter_acc.password,
              ACC_PASSWORD_SIZE); // INSECURE AS HELL
...
bool acc_login(Account *acc) {
  ...
  prompt_storable_input(credentials_acc.password, ACC_PASSWORD_SIZE,
                        L"Password");
  ...
  for (int i = 0; i < num_accs; i++) {
    ...
    bool same_password =
        wcscmp(iter_acc.password, credentials_acc.password) == 0;
...
//	Account dst_acc;
//	while (true) {
//		prompt_storable_input(dst_email, ACC_EMAIL_SIZE, L"Destinaton
// account (email)"); 		if (acc_find_by_email(dst_email, &dst_acc) !=
// NULL) { 			break; 		} else {
...
//		if (amount == 0.0f) { // wcstof returns 0.0f on error (awful
// decision btw,
//		        	      // cause what if trying to actually parse
//"0.0f" to float 			log_warning(L"Can't parse tha number,
// try again...\n"); 			continue; 		} else if (
// errno == ERANGE) { 			log_warning(L"Can't represent value, try
// again...\n "); 			continue; 		} else {
// break;
//		}
//	}
...
//		log_warning(L"Can't transfer more money than you have,
// dummy\n"); 		log_warning(L"Returning to menu..."); 		return;
//	}
//
