...
Account *csv_get_accs(Account accs[], int size) {
  ...
  while (fgetws(fbuff, sizeof fbuff, accounts) != NULL) {
    ...
    if (lines > 0 &&
        num_accs <=
            size) { // so we don't read first line which has column names
...
//			log_error(L"Trouble setting file position indicator on
//accounts.csv\n"); 			fclose(accounts); 			return NULL;
//		}
//
//
//		errno = 0;
...
//				wchar_t * state_tkzr;
//				wchar_t *sep = L",";
//				wchar_t *token = wcstok(fbuff, sep,
//&state_tkzr); 				int field_count = 0; 				while(token != NULL) { 					switch (field_count)
//{ 						case 0: 							wprintf(L"Chekinf if [%ls] matches with [%ls]\n", token,
//acc->email); 							bool matches = wcscmp(acc->email, token) == 0; 							if (matches) {
//
//								log_debug(L"ONE
//ACCOUNT MATCHES\n");
//
...
//									log_error(L"Trouble
//setting file position indicator on accounts.csv\n"); 									fclose(accounts); 									return
//NULL;
//								}
//
//								found = true;
...
//lbuff[128];
//
//								log_debug(L"CREATING
//RECORD\n");
//
//								int sfmt_rs =
//swprintf( 									lbuff, 									128, 									L"%ls,%ls,%.2f,\n", 									acc->email, acc->password,
//acc->balance
//								);
//								log_debug(L"record
//to be written: %ls", lbuff); 								if (sfmt_rs < 0) { 									log_error(L"Couldn't format
//data in order to persist it\n"); 									fclose(accounts); 									return NULL;
//								}
//
//								log_debug(L"WRITTING
//RECORD\n");
//
//								//int appnd_rs =
//fwprintf(
...
//								//if(appnd_rs <
//0) {
//									//log_error(L"Couldn't
//write data to accounts file\n");
//									//fclose(accounts);
//									//return
//NULL;
//								//}
...
//								//}
//
//								log_debug(L"RETURNING
//PERSISTED ACCOUNT SUCCESSFULLY\n"); 								return acc;
//
//							}
//							break;
//						case 1:
...
//			cpos = ftell(accounts);
//			if (cpos == -1) {
//				log_error(L"Couldn't get file position indicator
//for accounts.csv\n");
//			}
//			crrnt_line++;
//
...
//		}
//		if (!found) {
//			log_error(L"Couldn't found account, though it was
//previously found...\n"); 			fclose(accounts); 			return NULL;
//		}
//		fclose(accounts);
//		return acc;
